...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if center_y <= int(3*height/12+height/30) and center_x <= int(3*width/12+width/30):
                #추적할 차량의 class name과 일치할 경우 추적차량의 예상경로는 상좌
                if class_name == tyype:
                    direction1 = "Left"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
            elif center_y <= int(3*height/12+height/30) and center_x >= int(8*width/12-width/30) and center_x <= int(12*width/12+width/30): #상우
                #추적할 차량의 class name과 일치할 경우 추적차량의 예상경로는 상우
                if class_name == tyype:
                    direction1 = "Right"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
        ...
        cv2.putText(img, 'CCTV_ID:320', (0,30), 16, 0.6, (0,255,255),2)
        if direction1 == "Left":
            cv2.putText(img, 'Left', (100,130), 0, 0.4, (0,0,255),2)
            cv2.putText(img, 'Right', (450,80), 0, 0.4, (0,255,0),2)
        elif direction1 == "Right":
            cv2.putText(img, 'Left', (100,130), 0, 0.4, (0,255,0),2)
            cv2.putText(img, 'Right', (450,80), 0, 0.4, (0,0,255),2)
        else:
            cv2.putText(img, 'Left', (100,130), 0, 0.4, (0,255,0),2)
            cv2.putText(img, 'Right', (450,80), 0, 0.4, (0,255,0),2)

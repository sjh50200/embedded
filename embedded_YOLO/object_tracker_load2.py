...
def main(_argv):
    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if center_y >= int(9*height/12+height/30) and center_x <= int(6*width/12+width/30):
                if class_name == tyype:  
                    direction1 = "Straight"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
            elif center_y <= int(3*height/12+height/30) and center_x >= int(6*width/12-width/30) and center_x <= int(12*width/12+width/30):
                if class_name == tyype:
                    direction1 = "Back"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
        ...
        cv2.putText(img, 'CCTV_ID:155', (0,30), 16, 0.6, (0,255,255),2)
        if direction1 == "Straight":
            cv2.putText(img, 'Straight', (60,270), 0, 0.4, (0,0,255),2)
            cv2.putText(img, 'Back', (600,130), 0, 0.4, (0,255,0),2)
        elif direction1 == "Back":
            cv2.putText(img, 'Straight', (60,270), 0, 0.4, (0,255,0),2)
            cv2.putText(img, 'Back', (600,130), 0, 0.4, (0,0,255),2)
        else:
            cv2.putText(img, 'Straight', (60,270), 0, 0.4, (0,255,0),2)
            cv2.putText(img, 'Back', (600,130), 0, 0.4, (0,255,0),2)

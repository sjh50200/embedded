    ...
    while True:
        ...
        for track in tracker.tracks:
            ...
            if str(class_name) == "Attacker":
                ...
                #attacker_type = "HYNDAI_SONATA"
                if int(time_car) < int(135):
            ...
            if center_y <= int(4*height/12+height/30) and center_x <= int(6*width/12+width/30):
                if class_name == tyype:
                    direction1 = "Straight"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
            elif center_y >= int(3*height/6+height/30) and center_x >= int(8*width/12-width/30):
                if class_name == tyype:
                    direction1 = "Back"
                    data = str(attacker_type) + " " + str(attacker_color) + " " + str(direction1) + " " + str(accident_time) + "\n"
                    report1.write(data)
        ...
        if direction1 == "Straight":
            cv2.putText(img, 'Straight', (600,140), 0, 0.6, (0,0,255),2)
            cv2.putText(img, 'Back', (1500,900), 0, 0.6, (0,255,0),2)
        elif direction1 == "Back":
            cv2.putText(img, 'Straight', (600,140), 0, 0.6, (0,255,0),2)
            cv2.putText(img, 'Back', (1500,900), 0, 0.6, (0,0,255),2)
        else:
            cv2.putText(img, 'Straight', (600,140), 0, 0.6, (0,255,0),2)
            cv2.putText(img, 'Back', (1500,900), 0, 0.6, (0,255,0),2)
